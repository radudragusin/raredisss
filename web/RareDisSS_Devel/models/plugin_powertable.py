...
class PowerTable(TABLE):
    ...
    def __init__(self):
        ...
        truncate = powerTable.get('truncate',60)
        ...
        columnsearch = powerTable.get('columnsearch', True)
...
powerTable.defaultlanguage =  {'sLengthMenu': str(T('Display _MENU_ entries')),
                        'sZeroRecords': str(T('Nothing found. Please try another query.')),
...
def plugin_powertable():
    ...
    /*click on image to show details*/
                    ...
                    //alert(nTr.firstChild.firstChild);
                    if ( this.className == 'row_selected' )
                    {
                        /* This row is already open - close it */
                        if ((rid / 2) == (rid /2 | 0)) 
                            this.className = 'even clickable';
                        else
                            this.className = 'odd clickable';
                        oTable.fnClose( nTr );
                        img.src = "../static/plugin_powertable/images/details_open.png";
                        img.title = "Click for details";
                        img.alt =  "Open Details";
                    }
                    else
                    {
                        /* Open this row */
                        this.className = 'row_selected';
                        img.src = "../static/plugin_powertable/images/details_close.png";
                        img.title = "Click to close details";
                        img.alt =  "Close Details";
                        oTable.fnOpen( nTr, fnFormatDetails(oTable,nTr,key,rid), 'details' );
                        ajax('%(detailscallback)s', ['dt_'+rid,'dtcols_'+rid], 'target'+rid+'div');
                    }
                } );
    ...
    if powerTable.columnsearch:
        ...
        $("dataTables_filter input").keyup( function () {
                    ...
                    //alert("test");
                    //alert($("tfoot input").index(this) + %(index)s + 1);
                } );
